...
namespace winui_local_movie
{
    public sealed partial class AllVideosPage : Page
    {
        ...
        // 新增：在 DataTemplate 根上处理 Tapped，避免复选框触发播放
        private async void TempListItem_Tapped(object sender, TappedRoutedEventArgs e)
        {
            if (_isMultiSelectMode)
                return;

            // 如果点击来源是交互元素（例如列表选择复选框、按钮等），不要播放
            if (IsFromInteractiveElement(e.OriginalSource))
                return;

            if (sender is FrameworkElement fe && fe.DataContext is VideoModel video)
            {
                await PlayVideoAsync(video);
            }
        }
